            ...
            {sources.map((source) => (
              ...
              >
                <div className={`h-10 w-10 rounded-lg ${source.color} flex items-center justify-center text-lg mb-3`}>
                  {source.id === "notion" ? <NotionLogo className="h-5 w-5" /> : source.id === "google_doc" ? <GoogleDocsLogo className="h-5 w-5" /> : source.id === "pdf" ? <FileUp className="h-5 w-5 text-muted-foreground" /> : source.icon}
                </div>
